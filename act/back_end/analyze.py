...
def dispatch_tf(L, before, after, net):
    ...
    if k=="CONV1D": return tf_conv1d(L, before[L.id].bounds)
    if k=="CONV3D": return tf_conv3d(L, before[L.id].bounds)
    if k=="CONVTRANSPOSE2D": return tf_convtranspose2d(L, before[L.id].bounds)
    ...
    if k=="RELU6":   return tf_relu6(L, before[L.id].bounds)
    if k=="HARDTANH": return tf_hardtanh(L, before[L.id].bounds)
    if k=="HARDSIGMOID": return tf_hardsigmoid(L, before[L.id].bounds)
    if k=="HARDSWISH": return tf_hardswish(L, before[L.id].bounds)
    if k=="MISH":    return tf_mish(L, before[L.id].bounds)
    if k=="SOFTSIGN": return tf_softsign(L, before[L.id].bounds)
    ...
    # Tensor operations
    if k=="RESHAPE": return tf_reshape(L, before[L.id].bounds)
    if k=="TRANSPOSE": return tf_transpose(L, before[L.id].bounds)
    if k=="SQUEEZE": return tf_squeeze(L, before[L.id].bounds)
    if k=="UNSQUEEZE": return tf_unsqueeze(L, before[L.id].bounds)
    if k=="TILE":    return tf_tile(L, before[L.id].bounds)
    if k=="EXPAND":  return tf_expand(L, before[L.id].bounds)
